...
- Split apps keep `index.html` small and load `style.css` plus `js/*.js` in order through a loader that passes the page's `bcv` as `?v=`: `Drills`, `PseudocodeReference` (`interpreter`, `topics`, `app`), `TraceTablePractice` (`data`, `progress`, `ui`, `support`, `practice`, `generator`, `embed`, `main`), `LogicCircuits` (`gates`, `questions`, `board`, `wires`, `scoring`, `support`, `menu`, `help`), `PythonGame` (Python Defense: `powerups`, `data`, `stats`, `play`, `loader`, `answers`, `exams`, `runner`, `interpreter`, `support`, `menu`; its 52 levels, 7 stage exams and 10 GCSE question sets are JSON in `content/`, carried over from the old `jquinney-hue/pythongamejsons` repo and checked by check-site), `TurboBot/pybot.html` (PyBot: `levels`, `state`, `embed`, `setup`, `level`, `world`, `run`; the TurboBot overlay opens it with `pybotv` instead of `bcv`, pinned in `assets/js/turbobot.js`), `PseudocodeFarmer` (`catalogue`, `interpreter`, `missions`, `tutorials`, then the game in the order it runs: `state`, `scene`, `plants`, `code`, `machines`, `save`, `farm`, `rocket`, `hud`, `cloud`, `sandbox`, `loop`). The files share one scope in load order, so a file may only use what an earlier file declared at load time; a function declared in a later file exists only once that file has loaded, so load-time code may not call it (event handlers can). Split a new app the same way once it passes about 1,000 lines.
- TurboWarp: `scratch/` is the TurboWarp build; `scratch/editor.html` loads the PyScratch, FlowScratch and TurboBot overlays from the root `assets/js/` folder, each active only with its URL flag. With `?scratchcheck` it loads the Scratch Challenges files instead (`assets/js/scratchcheck-kit.js`, `-tester`, `-challenges-a`, `-challenges-b`, `scratchcheck.js`, and `-solutions` only with `&selftest`), each with the page's `bcv`; see the shell's `docs/scratch-challenges.md`. Their `?v=` tags in `scratch/editor.html` are pinned by hand, as are `psv` / `fsv` / `tbv` in the wrappers: bump them when an overlay changes. `PyScratch/`, `FlowScratch/` and `TurboBot/` are thin wrappers (`TurboBot/pybot.html` is PyBot).
...
- `tools/`: lesson builders (`tools/build_y10_2_1_l1.mjs` writes `LessonData/y10-2-1-l1.json`; `tools/build_y6_scratch.mjs` writes all six Year 6 lessons: edit the builder, not the JSON) and interaction tests.
...
- **No emoji**: icons are SVG line drawings in `currentColor`. check-site fails on emoji anywhere here except game art in an app's own `assets/` folder.
